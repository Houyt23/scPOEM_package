#'@title PGN_Lasso
#'
#'@description Make peak-gene net via Lasso.
#'@name PGN_Lasso
#'@import Matrix
#'@import glmnet
...
#' net_Lasso <- PGN_Lasso(input_single$X,
#'                        input_single$Y,
...
PGN_Lasso <- function(X, Y, gene_data, neibor_peak, dirpath, count_device, rebuild_PGN_Lasso){
  if (!rebuild_PGN_Lasso){
    cat("Load peak-gene network constructed by Lasso\n")
    net_lasso <- readMM(file.path(dirpath, "test/PGN_Lasso.mtx"))
    cat("\n")
    return(net_lasso)
  }
  cat("Construct peak-gene network via Lasso from scratch...\n")
  ...
  cat("PGN_Lasso is saved in:", file.path(dirpath, "test/PGN_Lasso.mtx"), "\n")
  cat("\n")
  return(net_lasso)
}
