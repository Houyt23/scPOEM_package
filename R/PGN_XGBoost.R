#'@title PGN_XGBoost
#'
#'@description Make peak-gene net via XGBoost.
#'@name PGN_XGBoost
#'@import Matrix
...
#' net_XGB <- PGN_XGBoost(input_single$X,
#'                        input_single$Y,
...
PGN_XGBoost <- function(X, Y, gene_data, neibor_peak, dirpath, count_device=1, rebuild_PGN_XGB=T) {
  if (!rebuild_PGN_XGB){
    cat("Load peak-gene network constructed by XGBoost\n")
  ...
  cat("Construct peak-gene network via XGBoost from scratch...\n")
  ...
  cat("PGN_XGB is saved in:", file.path(dirpath, "test/PGN_XGB.mtx"), "\n")
  cat("\n")
  return(net_XGB)
